...
#define LOG_TAG "CreateSubtitleComponentEntry"
...
const char *kSchedulerPrefix = "subtitle_scheduler";
...
extern "C" {
  OMX_API OMX_ERRORTYPE CreateComponentEntry(
    ...
    const char *subtitledecoderPrefix = "subtitle_decoder";

    OMX_LOGD("Omx Create Component %s", componentName);
    if (!strncmp(componentName, kCompNamePrefix, kCompNamePrefixLength)) {
      if (NULL != strstr(componentName, subtitledecoderPrefix)) {
        err = OmxSubtitleDecoder::createComponent(handle,
            componentName, appData, callBacks);
        return err;
      } else if (NULL != strstr(componentName, kSchedulerPrefix)) {
        err = OmxSubtitleScheduler::createComponent(handle,
            componentName, appData, callBacks);
        return err;
      }
    } else {
