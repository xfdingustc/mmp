...
#define LOG_TAG "BerlinOmxComponent"
...
static OMX_ERRORTYPE SetParameterWrapper(
  ...
  return getBase(hComponent)->setParameter(
...
static OMX_ERRORTYPE ComponentRoleEnumWrapper(
  ...
  return getBase(hComponent)->componentRoleEnum(cRole, nIndex);
}
...
static OMX_ERRORTYPE ComponentTunnelRequestWrapper(
  ...
  return getBase(hComponent)->componentTunnelRequest(
      nPort,
      hTunneledComp,
      nTunneledPort,
      pTunnelSetup);
...
static OMX_ERRORTYPE GetComponentVersionWrapper(
  ...
  return getBase(hComponent)->getComponentVersion(
      pComponentName,
      pComponentVersion,
      pSpecVersion,
      pComponentUUID);
...
static OMX_ERRORTYPE SetCallbacksWrapper  (
  ...
  return getBase(hComponent)->setCallbacks(
      pCallbacks,
      pAppData);
}
...
void OmxComponent::postFillBufferDone(OMX_BUFFERHEADERTYPE *bufHdr) {
  OMX_LOGV("postFillBufferDone %p", bufHdr);
