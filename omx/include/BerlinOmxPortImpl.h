...
#define CHECKOMXERRNOTRETURN(err) \
do { \
  if (OMX_ErrorNone != err) { \
    OMX_LOGE("%s %d error %d", __FUNCTION__, __LINE__, err); \
  } \
...
public:
  ...
  OMX_BUFFERHEADERTYPE * returnCachedbuffer();
  ...
  // virtual OmxPortFormat getPortFormat() {return mDefinition.format;};
...
protected:
  ...
  KDThreadMutex *mBufferMutex;
  KDThreadSem *mBufferSem;
...
class OmxClockPort : public OmxPortImpl {
public:
};

class OmxTextPort : public OmxPortImpl {
public:
};
} // namespace berlin
#endif // BERLIN_OMX_PORT_IMPL_H_
