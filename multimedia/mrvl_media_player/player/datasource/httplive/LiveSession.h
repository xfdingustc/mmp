...
{
  HLS_UNKNOWN = 0,
  HLS_PLAYING_BANDWIDTH,
  HLS_REALTIME_SPEED,
  HLS_DOWNLOAD_PROGRESS_IN_PERCENT,
  HLS_DOWNLOAD_PROGRESS_IN_BYTES,
  HLS_DOWNLOAD_ERROR,
  HLS_DOWNLOAD_PLAYLIST_ERROR,
  HLS_DOWNLOAD_TS_ERROR,
  HLS_Got_Socket_IP_Address,
  HLS_UNSUPPORT,
} HLSEvent;

class HttpLiveCbTarget {
 public:
  virtual ~HttpLiveCbTarget() {};
  virtual bool OnHttpLiveEvent(HLSEvent what, int extra) = 0;
};
...
struct LiveSession : public AHandler {
    ...
    void registerCallback(HttpLiveCbTarget  *cb_target) { mHttpLiveCbTarget = cb_target; }

    int64_t getLastSeekMediaTime() { return mLastSeekMediaTime; }
    bool reportRealTimeSpeed();
...
private:
    ...
    sp<ChromiumHTTPDataSource> mHTTPDataSource;
    ...
    int64_t mLastSeekMediaTime;

    HttpLiveCbTarget *mHttpLiveCbTarget;
