...
LiveSession::LiveSession(uint32_t flags, bool uidValid, uid_t uid)
    : mFlags(flags),
      ...
      mLastSeekMediaTime(0),
      mHttpLiveCbTarget(NULL) {
    HLS_LOGD("ENTER, begin to create LiveSession");

    mDataSource = new LiveDataSource;

    mHTTPDataSource = new ChromiumHTTPDataSource();

    if (mUIDValid) {
        mHTTPDataSource->setUID(mUID);
    }

    HLS_LOGD("EXIT, LiveSession is created");
}
...
void LiveSession::onMessageReceived(const sp<AMessage> &msg) {
    switch (msg->what()) {
        ...
        case kWhatNetworkSpeed:
            reportRealTimeSpeed();
            {
              // Every 6s, report the network speed
              sp<AMessage> msg = new AMessage(kWhatNetworkSpeed, id());
              msg->post(6000000ll);
            }
            break;
...
void LiveSession::onConnect(const sp<AMessage> &msg) {
    ...
    HLS_LOGI("onConnect <URL %s>", url.c_str());
    ...
    if (playlist->isVariantPlaylist()) {
        HLS_LOGD("This is a adaptive playlist.");
        for (size_t i = 0; i < playlist->size(); ++i) {
            ...
            HLS_LOGD("bandwidth %d: %d", i, item.mBandwidth);
    ...
    sp<AMessage> speed_msg = new AMessage(kWhatNetworkSpeed, id());
    speed_msg->post(1000000ll);
...
status_t LiveSession::fetchFile(
    ...
    *out = NULL;
    ...
    sp<ChromiumHTTPDataSource> source;
...
rinse_repeat:
        ...
        if (mPlaylist->isComplete() || mPlaylist->isEvent()) {
            ...
            if (index < mPlaylist->size()) {
                ...
                if (newSeqNumber != mSeqNumber) {
                    HLS_LOGI("seeking to seq no %d", newSeqNumber);

                    mSeqNumber = newSeqNumber;

                    mLastSeekMediaTime = getSegmentStartTimeUs(mSeqNumber);
                    HLS_LOGI("mLastSeekMediaTime = "TIME_FORMAT"(%lld us)",
                        TIME_ARGS(mLastSeekMediaTime), mLastSeekMediaTime);
    ...
    if (bandwidthChanged) {
        int bandwidth = 0;
        if (bandwidthIndex < mBandwidthItems.size()) {
          bandwidth = mBandwidthItems.itemAt(bandwidthIndex).mBandwidth;
        }
        if (mHttpLiveCbTarget) {
          mHttpLiveCbTarget->OnHttpLiveEvent(HLS_PLAYING_BANDWIDTH, bandwidth);
        }
    }
...
void LiveSession::onMonitorQueue() {
    if (mSeekTimeUs >= 0
        ...
        HLS_LOGD("onDownloadNext()");
        onDownloadNext();
    } else {
        HLS_LOGD("postMonitorQueue(1000000ll), becasue mDataSource->countQueuedBuffers()"
            " = %d, kMaxNumQueuedFragments = %d",
            mDataSource->countQueuedBuffers(), kMaxNumQueuedFragments);
...
status_t LiveSession::decryptBuffer(
    ...
    if (index >= 0) {
        ...
        sp<ChromiumHTTPDataSource> keySource = new ChromiumHTTPDataSource();
...
void LiveSession::onSeek(const sp<AMessage> &msg) {
    ...
    HLS_LOGD("seek to %lld us", timeUs);
...
bool LiveSession::reportRealTimeSpeed() {
  int32_t bandwidthBps = 0;
  if (mHTTPDataSource != NULL
          && mHTTPDataSource->estimateBandwidth(&bandwidthBps)) {
      HLS_LOGD("bandwidth estimated at %.2f kbps", bandwidthBps / 1024.0f);
      // Consider only 80% of the available bandwidth usable.
      bandwidthBps = (bandwidthBps * 8) / 10;
      if (mHttpLiveCbTarget) {
        mHttpLiveCbTarget->OnHttpLiveEvent(HLS_REALTIME_SPEED, bandwidthBps);
      }
  } else {
      HLS_LOGE("no bandwidth estimate.");
  }
